...
async function importAll349StaffComplete() {
  try {
    console.log('🚀 Starting comprehensive import of ALL 349 staff members...');
    console.log(`📊 Processing ${all349StaffData.length} employees from the complete list...`);
    ...
    const batchSize = 50;
    const totalBatches = Math.ceil(all349StaffData.length / batchSize);
    
    console.log(`📦 Processing in ${totalBatches} batches of ${batchSize} employees each...`);

    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      const startIndex = batchIndex * batchSize;
      const endIndex = Math.min(startIndex + batchSize, all349StaffData.length);
      const batch = all349StaffData.slice(startIndex, endIndex);
